...
class MMF13(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        F[:, 0] = X1
        t = X2 + sqrt(X3)
        g = 2 - exp(-2 * log10(2) * ((t - 0.1) / 0.8) ** 2) * sin(self.np * pi * t)
        F[:, 1] = g / X1
    ...
    def _calc_pareto_front(self, n_pareto_points=500):
        ...
        PF[:, 0] = F1
        PF[:, 1] = (2-exp(-2*log10(2)*((1/(2*self.np)-0.1)/0.8)**2)*sin(self.np*pi*(1/(2*self.np)))**6) / F1
    ...
    def _calc_pareto_set(self, n_pareto_points=500):
        ...
        X2 = np.linspace(0.1, 1 / (2 * self.np) - 0.01, h)
